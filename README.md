...
3. 根据实际链路设置每包开销和最小包长。FTTH 封装和计费方式不明时，可先使用默认的经验值 `44/84`，详见下方说明。
...
服务直接使用 `overhead <数值> mpu <数值>`，当前默认值为 **overhead 44、MPU 84**，上传和下载共用这组参数。`overhead` 是每包补偿，`mpu` 是补偿后的最小计费长度。

本项目不启用 ATM/PTM 补偿，计费关系可理解为：
...
以下数值以补偿**以太网线上占用**为目标，必须满足表中的包长前提：

| CAKE 的基础包长与尚需补偿的封装                                 | overhead | MPU |
| --------------------------------------------------------------- | -------: | --: |
| 基础包长为 IP 包，补偿以太网开销，无 VLAN                       |       38 |  84 |
| 同上，另有一层未计入的 VLAN                                     |       42 |  84 |
| 基础包长为 IP 包，还需补偿 PPPoE/PPP，无 VLAN                   |       46 |  84 |
| 同上，另有一层未计入的 VLAN                                     |       50 |  84 |
| 基础包长已包含 PPPoE/PPP 的 8 字节，仅需补偿以太网开销，无 VLAN |       38 |  84 |

以太网开销为帧头 14、FCS 4、前导码 8 和帧间隙等效长度 12，共 38 字节；PPPoE/PPP 为 8 字节，每层 VLAN 为 4 字节。最小以太网帧 64 字节，加上前导码和帧间隙的占用，对应 MPU 84。`18/64` 则是 CAKE 的 DOCSIS 预设，不能仅因为使用网线就选 MPU 64。预设和封装定义见 [CAKE 手册](https://github.com/iproute2/iproute2/blob/main/man/man8/tc-cake.8#L298)。

### 如何为当前接法选择参数

- **上级设备拨号、本机 DHCP：** `38/84` 可描述无 VLAN 的本地以太网段，但上级设备之后的封装和运营商限速计费方式仍可能不同。
- **本机 PPPoE、整形挂在物理 WAN：** Linux 常规 PPPoE 发送路径将网络头指向 PPPoE 头，据此推导，基础包长可已包含 PPPoE/PPP 的 8 字节，不能直接套用以纯 IP 包为基础的 `46/84`。这是对代码路径的分析，不代表对所有固件、驱动及下载 IFB 路径的实测确认。参见 [Linux PPPoE 实现](https://github.com/torvalds/linux/blob/v6.12/drivers/net/ppp/pppoe.c)。
- **存在 VLAN：** 按未计入基础包长的标签数量补偿，每层 4 字节；不要仅根据接口名决定数值。
- **FTTH 封装和计费方式不明：** 可先采用 `44/84`。这是 SQM 维护者建议的经验起点，不是精确值，也不是所有封装的开销上限；适度高估通常比低估更利于控制满载延迟。参见 [SQM 维护者说明](https://lists.openwrt.org/pipermail/openwrt-devel/2022-November/039814.html)。

**当前默认 `44/84` 是 FTTH 的经验起点，不是所有线路的精确值。** 精确设置还需要运营商的计费口径，以及上传和下载整形位置实际计入的头部信息。
